...
listTable.addEventListener('click', (e) => {
  ...
  if (role === 'remove-cookie') {
    ...
    document.cookie = `${cookieName}=deleated; max-age=0`;
    updateTable();
  }
});
...
function updateTable() {
  ...
  for (const [name, value] of cookiesMap) {
    if (
      filterValeu &&
      !name.toLowerCase().includes(filterValeu.toLocaleLowerCase()) &&
      !value.toLowerCase().includes(filterValeu.toLocaleLowerCase())
    ) {
      continue;
    }
    ...
    tr.append(nameTD, valueTD, remoteTD);
    remoteTD.append(removeButton);
